...
interface ManualConnectDialogProps {
  ...
  onConnected: () => void;
  /** When set, dialog operates in edit mode — updates existing channel via PATCH */
  editAccountId?: string;
  /** Pre-filled values for edit mode (channelId, displayName — not the token) */
...
export function ManualConnectDialog({
  ...
  const isEditMode = !!editAccountId;
  const [step, setStep] = useState<'form' | 'qr' | 'success'>('form');
  const [loading, setLoading] = useState(false);
  const [values, setValues] = useState<Record<string, string>>(initialValues || {});
  const [embedCode, setEmbedCode] = useState<string | null>(null);

  // Re-initialize values when initialValues change (e.g. opening edit for a different channel)
  useEffect(() => {
  ...
  const handleSubmit = async () => {
    // Validate required fields (in edit mode, accessToken is optional — keep existing if blank)
    for (const field of config.fields) {
      if (field.required && !values[field.key]?.trim()) {
        if (isEditMode && field.key === 'accessToken') {
          // In edit mode, token can be left blank to keep the existing one
          continue;
        }
    ...
    try {
      ...
      if (isEditMode) {
        // Edit mode: PATCH existing channel
        const patchBody: Record<string, string> = {};
        if (values.displayName?.trim()) patchBody.displayName = values.displayName.trim();
        if (values.channelId?.trim()) patchBody.channelId = values.channelId.trim();
        if (values.accessToken?.trim()) patchBody.accessToken = values.accessToken.trim();

        const response = await fetch(`/api/channels/${editAccountId}`, {
          method: 'PATCH',
        ...
        setStep('success');
        toast.success(`${config.name} credentials updated!`);
      } else {
        // Create mode: POST new channel
      ...
      toast.error(
        error instanceof Error ? error.message : isEditMode ? 'Update failed' : 'Connection failed'
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[550px] p-0 overflow-hidden">
        ...
        <div
          className={cn('relative px-6 py-8 text-white', `bg-gradient-to-br ${config.gradient}`)}
        >
          {/* Decorative pattern */}
          <div
            ...
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <DialogTitle className="text-2xl font-bold text-white">
                  {isEditMode ? `Edit ${config.name}` : `Connect ${config.name}`}
                </DialogTitle>
        ...
        <AnimatePresence mode="wait">
          {step === 'qr' ? (
            ...
            <motion.div
              ...
              className="p-6 space-y-6"
            >
              {/* Instructions (hidden in edit mode) */}
              {!isEditMode && (
                  ...
                  <div key={field.key} className="space-y-2">
                    ...
                    {field.type === 'textarea' ? (
                      ...
                      <Input
                        ...
                        type={field.type}
                        placeholder={
                          isEditMode && field.key === 'accessToken'
            ...
            >
              <div className="text-center py-4">
                ...
                <h3 className="text-xl font-semibold mb-2">
                  {isEditMode ? `${config.name} Updated!` : `${config.name} Connected!`}
                </h3>
                <p className="text-muted-foreground">
                  {isEditMode
                    ? 'Credentials have been updated. The channel will reconnect automatically.'
                    : 'Messages will now appear in your unified inbox.'}
                </p>
                {channelType === 'whatsapp' && (
                  <p className="text-sm text-muted-foreground mt-2">
                    Auto-reply is off by default. Enable it in channel settings to let the AI
