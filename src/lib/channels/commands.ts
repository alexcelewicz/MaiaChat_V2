...
async function handleModels(context: CommandContext): Promise<CommandResult> {
    ...
    for (const provider of availableProviders) {
        const isConfigured = ['anthropic', 'openai', 'google', 'xai', 'openrouter'].includes(provider.id)
            ? '✓'
            : '(local)';
...
async function handleRAG(args: string[], context: CommandContext): Promise<CommandResult> {
    ...
    if (enabled) {
        const [doc] = await db.select()
            .from(documents)
            .where(and(
                eq(documents.userId, context.userId),
    ...
    if (enabled) {
        ...
        if (!hasGeminiFile) {
            return {
                isCommand: true,
                handled: true,
                response: 'Gemini file search enabled, but no Gemini files were found. Upload documents to Gemini in the web UI first.',
...
async function handleNewChat(context: CommandContext): Promise<CommandResult> {
    try {
        ...
        const channelConversations = await db.select()
            .from(conversations)
            .where(eq(conversations.userId, context.userId));

        let clearedCount = 0;
        for (const conv of channelConversations) {
            const metadata = conv.metadata as { channelThreadKey?: string; channelType?: string; channelId?: string } | null;
    ...
    if (command === 'set') {
        ...
        if (agentNames.length === 0) {
            return {
                isCommand: true,
                handled: true,
                response: 'Usage: `/multiagent set <agent1> <agent2> ...`\nProvide agent names separated by spaces.',
            };
        }
        ...
        const userAgents = await db.select()
            .from(agents)
            .where(eq(agents.userId, context.userId));

        const matchedAgents: string[] = [];
        const notFound: string[] = [];

        for (const name of agentNames) {
            const agent = userAgents.find(a =>
