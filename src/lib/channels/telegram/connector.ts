...
export class TelegramConnector extends ChannelConnector {
  ...
  /**
   * Force-close any existing polling session for this bot token.
   * Calls deleteWebhook (which also clears getUpdates sessions) and
   * logOut to release the token, then waits briefly for Telegram to
   * acknowledge the session is gone.
   */
  private async clearPreviousSession(botToken: string): Promise<void> {
    try {
      // deleteWebhook with drop_pending_updates clears any active
      // getUpdates long-poll or webhook that another process holds
      await fetch(`https://api.telegram.org/bot${botToken}/deleteWebhook`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ drop_pending_updates: shouldDropPendingUpdates() }),
        signal: AbortSignal.timeout(5000),
      });

      // logOut invalidates the current session so the next getUpdates
      // can claim it. Ignore errors — logOut may fail if no session exists.
      await fetch(`https://api.telegram.org/bot${botToken}/logOut`, {
        method: 'POST',
        signal: AbortSignal.timeout(5000),
      }).catch(() => {
        /* ignore */
      ...
      // Telegram docs recommend waiting ≥10 s after logOut before
      // starting a new session, but in practice 3-5 s suffices for
      // dev-reload scenarios.
      await new Promise((resolve) => setTimeout(resolve, 3000));
    } catch (err) {
      console.warn('[Telegram] Failed to clear previous session:', err);
    }
  }
  ...
  async connect(config: ChannelConfig): Promise<void> {
    ...
    // If we already hold a bot instance (e.g. HMR re-run), stop it first
    if (this.bot) {
      try {
        await this.bot.stop();
      } catch {
    ...
    try {
      ...
      if (startError instanceof Error && startError.message.includes('409')) {
        console.warn('[Telegram] Bot conflict detected (409). Clearing previous session...');
        await this.clearPreviousSession(config.credentials.botToken);

        // Retry after clearing the session
        try {
          await this.bot.start({
            drop_pending_updates: dropPendingUpdates,
            onStart: () => {
              console.log(
                `[Telegram] Bot started (after session clear, drop_pending_updates=${dropPendingUpdates})`
              );
            },
          });
        } catch (retryError) {
          console.error('[Telegram] Failed to start bot after clearing session:', retryError);
          throw retryError;
        }
      } else {
        throw startError;
  ...
  async sendMessage(
    ...
    if (chunks.length > 1) {
      console.log(
        `[Telegram] Split message into ${chunks.length} chunks (${content.length} chars total)`
      );
    }
  ...
  async editMessage(channelId: string, messageId: string, content: string): Promise<void> {
    ...
    try {
      await this.bot.api.editMessageText(channelId, parseInt(messageId), sanitizedContent, {
        parse_mode: 'HTML',
      });
    } catch (error) {
      ...
      await this.bot.api.editMessageText(channelId, parseInt(messageId), plainText);
    }
  }
