...
import { conversations, messages, channelAccounts, adminSettings, channelMessages, agents } from "@/lib/db/schema";
...
export async function runIsolatedAgent(config: IsolatedAgentConfig): Promise<IsolatedAgentResult> {
    ...
    try {
        ...
        if (sessionTarget === "main" && includeRecentMessages > 0 && channelAccountId) {
            const recentMessages = await getRecentMessages(userId, channelAccountId, includeRecentMessages);
        ...
        if (Object.keys(tools).length > 0) {
            ...
            if (hasDatetimeTools) {
                toolHints.push('You have datetime tools to get the current time. ALWAYS use datetime__current_time to get accurate time - never guess or approximate.');
            }
            if (hasWebSearch) {
                toolHints.push('You have web search tools to find current information online. Use them for any questions requiring up-to-date data.');
            }
        ...
        const taskResult = await executeTaskWithRetry(
            message,
            taskConfig,
            {
                model,
        ...
        console.log(`[IsolatedAgent] Task result: success=${taskResult.success}, attempts=${taskResult.attempts}, tools=${taskResult.toolsCalled.join(',')}`);

        // Handle task failure
        if (!taskResult.success) {
            console.warn(`[IsolatedAgent] Task "${taskName}" failed after ${taskResult.attempts} attempts: ${taskResult.failureReason}`);
            ...
            if (taskConfig.notifyTelegram && channelAccountId) {
                telegramNotificationSent = await notifyTaskFailure(userId, channelAccountId, taskName, taskResult);
            ...
            const skipDuplicateNotification = telegramNotificationSent && (!channel || channel === "telegram");
            let primaryDeliveryStatus: IsolatedAgentResult["primaryDeliveryStatus"] = deliver ? "failed" : "not_requested";

            if (taskConfig.notifyOriginalChannel && channelAccountId && !skipDuplicateNotification) {
                const failureMessage = `⚠️ **Task Failed:** ${taskName}\n\n` +
        ...
        afterAgentEnd(
            { userId, conversationId: taskId, input: message, systemPrompt: "", channelType: "scheduled", channelId: taskId },
            { output: fullResponse, tokensUsed: taskResult.tokensUsed, toolsCalled: taskResult.toolsCalled }
        ...
        if (sessionTarget === "isolated") {
            conversationId = await storeConversation(userId, taskId, taskName, message, fullResponse, consolidateConversations);
        }
...
async function deliverMessage(
    ...
    try {
        ...
        if (account.channelType === "telegram") {
            ...
            if (!chatId) {
                console.error("[IsolatedAgent] No valid Telegram chat ID found. The bot needs at least one message from a user to know where to send scheduled messages.");
                return undefined;
            }

            // Verify bot token is valid first
            const tokenPrefix = botToken.substring(0, 10);
            console.log(`[IsolatedAgent] Sending to Telegram chat ${chatId} with bot token starting with: ${tokenPrefix}...`);
        ...
        try {
            ...
            console.log(`[IsolatedAgent] Delivered message to ${account.channelType}:${to || account.channelId}`);
            return `${account.channelType}:${to || account.channelId}`;
        } catch (managerError) {
            console.error(`[IsolatedAgent] ChannelManager delivery failed for ${account.channelType}:`, managerError);
            return undefined;
        }
    } catch (error) {
    ...
    try {
        ...
        if (!account || account.channelType !== "telegram" || !account.accessToken) {
            ...
            if (chatId) {
                ...
                console.log(`[IsolatedAgent] Failure notification sent via fallback Telegram account to chat_id=${chatId}`);
                return true;
            }
            return false;
        }
