...
export function getModelWithKey(
  ...
  // Use the resolved ID from config (handles prefix normalization, e.g. "openrouter/org/model" → "org/model")
  return provider.getModel(modelConfig.id);
}
...
export function getModelConfig(modelId: string): ModelConfig | undefined {
  ...
  // Check for explicit openrouter/ prefix (e.g., "openrouter/minimax/minimax-m2.5")
  // Strip the prefix and route to OpenRouter — the real model ID is what follows
  if (modelId.startsWith('openrouter/')) {
    const strippedId = modelId.slice('openrouter/'.length);
    const orProvider = createOpenRouterProvider() as {
      getModelConfig?: (id: string) => ModelConfig | undefined;
    };
    if (orProvider.getModelConfig) {
      return orProvider.getModelConfig(strippedId);
    }
  }
...
export function getConfiguredProviders(): ProviderId[] {
  return (Object.keys(defaultProviders) as ProviderId[]).filter((id) => isProviderConfigured(id));
}
