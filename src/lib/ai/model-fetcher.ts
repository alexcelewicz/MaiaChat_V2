...
import { OPENAI_MODELS, ANTHROPIC_MODELS, GOOGLE_MODELS, XAI_MODELS, OPENROUTER_MODELS, OLLAMA_MODELS, LMSTUDIO_MODELS } from "./models";
...
function inferCapabilities(model: OpenRouterModel): ModelCapability[] {
    ...
    if (id.includes("gpt-4") || id.includes("gpt-3.5") || id.includes("claude") ||
        id.includes("gemini") || id.includes("mistral-large")) {
        caps.push("tools");
    }
    ...
    if (id.includes("o1") || id.includes("o3") || id.includes("deepseek-r1") ||
        name.includes("reasoning") || id.includes("qwq")) {
        caps.push("reasoning");
    }

    // Code models
    if (id.includes("code") || id.includes("deepseek") || id.includes("codestral") ||
        id.includes("claude") || id.includes("gpt-4")) {
        caps.push("code");
    }
...
export async function fetchOpenAIModels(apiKey: string): Promise<ModelConfig[]> {
    try {
        ...
        const chatModels = models.filter((m: { id: string }) =>
...
export async function fetchGoogleModels(apiKey: string): Promise<ModelConfig[]> {
    try {
        ...
        const chatModels = models.filter((m: { name: string }) =>
            m.name.includes("gemini")
        );

        return chatModels.map((m: { name: string; displayName?: string; inputTokenLimit?: number; outputTokenLimit?: number; description?: string }) => {
...
export async function fetchOllamaModels(): Promise<ModelConfig[]> {
    try {
        ...
        return models.map((m) => {
            ...
            const isReasoning = modelId.toLowerCase().includes("r1") || modelId.toLowerCase().includes("deepseek");
        ...
        return []; // Return empty if Ollama not running
    }
}
...
export async function fetchLMStudioModels(): Promise<ModelConfig[]> {
    try {
        ...
        console.warn("LM Studio not running or not accessible:", error);
        return []; // Return empty if LM Studio not running
    }
}
...
export async function fetchAllModels(
    ...
    const providers: ProviderId[] = ["openai", "anthropic", "google", "xai", "openrouter", "ollama", "lmstudio"];
    ...
    return Object.fromEntries(
        filteredResults.map((result) => [result.provider, result])
    ) as Record<ProviderId, ProviderModelsResult>;
}
