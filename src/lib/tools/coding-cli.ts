...
export const CodingCliInputSchema = z.object({
    ...
    timeout: z.number().default(180000).describe("Inactivity timeout in ms — kill if no output for this long (default 3 min)"),
    maxTimeout: z.number().default(1800000).describe("Hard max timeout in ms — safety cap regardless of activity (default 30 min)"),
...
function getEnhancedPATH(): string {
    ...
    const valid = extra.filter((p) => { try { return fs.existsSync(p); } catch { return false; } });
...
export async function executeCodingCLI(input: CodingCliInput): Promise<CodingCliResult> {
    ...
    const filesBefore = new Set(
        fs.existsSync(workDir)
            ? fs.readdirSync(workDir, { recursive: true }).map(String)
            : []
    );
