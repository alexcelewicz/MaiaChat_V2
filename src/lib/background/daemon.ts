...
import type { DaemonState, DaemonInfo, DaemonStatus, BackgroundServiceConfig } from './types';
...
class BackgroundDaemon {
  ...
  async start(): Promise<boolean> {
    ...
    try {
      ...
      // Register Remote Control cleanup on shutdown
      const { claudeRemoteControl } = await import('@/lib/services/claude-remote-control');
      this.onShutdown(async () => {
        await claudeRemoteControl.stopAll();
      });
  ...
  private async loadConfig(): Promise<BackgroundServiceConfig> {
    ...
    return {
      ...
      autoStartOnBoot:
        settings.backgroundAgentAutoStart ?? agentConfig.backgroundAgentAutoStart ?? false,
      heartbeatIntervalMs,
      staleThresholdMs,

      proactiveMessagingEnabled:
        settings.proactiveMessagingEnabled ?? agentConfig.proactiveMessagingEnabled ?? false,
      eventTriggersEnabled:
        settings.eventTriggersEnabled ?? agentConfig.eventTriggersEnabled ?? false,
      bootScriptsEnabled: settings.bootScriptsEnabled ?? agentConfig.bootScriptsEnabled ?? false,

      defaultProactiveMaxPerHour:
        rateConfig.proactiveMaxPerHour ?? settings.defaultProactiveMaxPerHour ?? 10,
      defaultProactiveMaxPerDay:
        rateConfig.proactiveMaxPerDay ?? settings.defaultProactiveMaxPerDay ?? 100,
      defaultTriggerMaxPerHour:
        rateConfig.triggerMaxPerHour ?? settings.defaultTriggerMaxPerHour ?? 60,
  ...
  private async startServices(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `[Daemon] Session manager initialized: ${result.recoverableTasks} recoverable, ${result.pausedTasks} paused`
      );
    } catch (error) {
