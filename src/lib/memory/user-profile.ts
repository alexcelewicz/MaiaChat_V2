...
// Configuration
const PROFILE_BASE_DIR = process.env.PROFILE_DIR || "/app/data/profiles";
...
export async function addUserFact(
    ...
    const existingFact = profile.facts.find(
        (f) => f.text.toLowerCase() === fact.text.toLowerCase()
    );
    if (existingFact) {
...
async function extractWithLLM(
    ...
    company?: string;
    interests?: string[];
    facts: Array<{ text: string; category: UserFact["category"]; confidence: UserFact["confidence"] }>;
} | null> {
    try {
...
export async function addUserProvidedInfo(
    ...
    if (info.communicationStyle !== undefined) profile.communicationStyle = info.communicationStyle || undefined;
    ...
    if (info.customInstructions && info.customInstructions.trim()) {
        ...
        profile.facts = profile.facts.filter(
            (f) => !f.text.startsWith("User instructions:")
        );
