...
// Configuration
const MEMORY_BASE_DIR = process.env.MEMORY_DIR || "/app/data/memory";
...
export async function getMemoryEntries(userId: string): Promise<{
    ...
    try {
        ...
        for (const indexEntry of index.entries.slice(-50)) { // Last 50 entries
            try {
...
async function archiveOlderEntries(userId: string): Promise<{
    ...
    const entriesToArchive = sortedEntries.slice(0, sortedEntries.length - ROLLING_WINDOW.keepRecentEntries);
    const entriesToKeep = sortedEntries.slice(sortedEntries.length - ROLLING_WINDOW.keepRecentEntries);

    if (entriesToArchive.length === 0) {
        return { archivedCount: 0 };
    }

    console.log(`[LocalMemory] Archiving ${entriesToArchive.length} older entries, keeping ${entriesToKeep.length} recent`);
    ...
    let currentOffset = Buffer.byteLength(`# MaiaChat Memory Log\n\nThis file contains conversation summaries and important information.\n\n---\n\n`, "utf-8");
...
async function consolidateAndUploadToGemini(userId: string): Promise<{
    ...
    console.log(`[LocalMemory] Pending files: ${files.length}, total size: ${(totalSize / 1024).toFixed(1)}KB`);

    // Check if we should upload (either enough size or too many files)
    const shouldUpload = totalSize >= GEMINI_UPLOAD.minFileSize || files.length >= GEMINI_UPLOAD.maxPendingFiles;

    if (!shouldUpload) {
        console.log(`[LocalMemory] Not enough content for upload yet (need ${(GEMINI_UPLOAD.minFileSize / 1024).toFixed(0)}KB or ${GEMINI_UPLOAD.maxPendingFiles} files)`);
        return { uploadedToGemini: false, filesConsolidated: 0, cleanedUpLocally: false };
    }
    ...
    const consolidatedContent = fileContents.map(f => f.content).join("\n\n" + "=".repeat(80) + "\n\n");
    const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
    const consolidatedFilename = `consolidated_memory_${timestamp}.md`;

    console.log(`[LocalMemory] Consolidating ${files.length} files (${(totalSize / 1024).toFixed(1)}KB) for Gemini upload`);
...
export async function appendToWorkingMemory(
    ...
    if (index.entries.length > ROLLING_WINDOW.archiveAfterEntries) {
        console.log(`[LocalMemory] Entry count (${index.entries.length}) exceeds threshold (${ROLLING_WINDOW.archiveAfterEntries}), archiving older entries`);
    ...
    try {
        ...
        for (const keyword of queryKeywords) {
            ...
            for (const entryId of matchingEntryIds) {
                ...
                const idf = Math.log(
                    (index.entries.length - matchingEntryIds.length + 0.5) /
                    (matchingEntryIds.length + 0.5) + 1
                );
