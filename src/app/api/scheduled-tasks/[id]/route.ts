...
function shouldRequireToolCallForTask(prompt: string): boolean {
  ...
  return /\b(weather|forecast|temperature|rain|wind|news|headline|latest|stock|price|exchange rate|today|current)\b/.test(
    normalized
  );
}
...
function resolveTaskExecution(
  ...
  const agentId =
    executionMode === 'agent' && typeof data.agentId === 'string' ? data.agentId : null;
  return {
    executionMode,
    agentId,
  ...
  try {
    ...
    if (body.prompt !== undefined) {
      ...
      updates.payload = {
        ...
        kind: 'agentTurn',
        message:
          typeof currentPayload.message === 'string' ? currentPayload.message : existing.prompt,
        deliver: true,
        executionMode: nextExecutionMode,
        agentId: nextExecutionMode === 'agent' ? nextAgentId : null,
      };
