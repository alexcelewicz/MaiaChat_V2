...
import { messages as messagesTable, conversations, usageRecords, agents, documents, users } from "@/lib/db/schema";
...
async function autoSaveMemory(
    ...
    try {
        ...
        console.log(`[Chat] Auto-saved memory for conversation ${conversationId.slice(0, 8)} (${msgs.length} messages)`);
    } catch (error) {
...
export async function POST(req: Request) {
    try {
        ...
        if (!modelConfig) {
            return new Response(
                JSON.stringify({ error: "Model not found", code: "MODEL_NOT_FOUND" }),
                { status: 400, headers: { "Content-Type": "application/json" } }
            );
        }
        ...
        const useRag = retrievalMode
            ? ["rag", "both"].includes(retrievalMode)
            : ragEnabled;
        ...
        const geminiFileIdsPromise = useGemini
            ? (geminiFileIds?.length
                ...
                : db.select({ metadata: documents.metadata })
                    .from(documents)
                    .where(and(
        ...
        if (memoryEnabled) {
            try {
                ...
                const [{ getLocalMemoryContext, searchAllLocalMemory }, memoryStoreModule] = await Promise.all([
                ...
                const [localMemory, searchResults, geminiMemory] = await Promise.all([
                    ...
                    (googleKey && trimmedContent)
                        ? memoryStoreModule.retrieveMemories(userId, googleKey, trimmedContent)
                            .catch((err: unknown) => { console.error("Gemini memory retrieval failed:", err); return ""; })
                        : Promise.resolve(""),
                ]);
                ...
                console.log(`[Chat] Memory context: local=${localMemory.length}chars, search=${localSearchResults.length}chars, gemini=${geminiMemory.length}chars`);
            } catch (err) {
        ...
        // 6. Save User Message (fire-and-forget, don't block)
        ...
        const effectiveModelConfig = agentModel ? getModelConfig(agentModel) || modelConfig : modelConfig;
...
- Be direct and get to the point quickly`,
        ...
        try {
            const [prefRow] = await db.select({ preferences: users.preferences })
                .from(users)
...
Perplexity models require a Perplexity API key to be configured in Settings.`,
        ...
        if (toolsEnabled) {
            ...
            let enabledToolIds = enabledTools?.length
                ? enabledTools
            ...
            for (const toolId of enabledToolIds) {
                ...
                tools[toolDef.id] = tool({
                    ...
                    execute: async (params) => {
                        console.log(`[Chat] Executing tool ${toolDef.id} with params:`, JSON.stringify(params).substring(0, 200));
                        try {
            ...
            for (const plugin of pluginRegistry.list()) {
                ...
                for (const pluginTool of plugin.manifest.tools || []) {
                    ...
                    tools[toolName] = tool({
                        ...
                        execute: async (params) => {
                            console.log(`[Chat] Executing skill ${toolName} with params:`, JSON.stringify(params).substring(0, 200));
                            try {
        ...
        if (toolsEnabled || skillsEnabled) {
            console.log(`[Chat] Tools enabled: ${toolsEnabled}, Skills enabled: ${skillsEnabled}, Registered tools: ${Object.keys(tools).join(", ") || "(none)"}`);
        }
        ...
        if (Object.keys(tools).length > 0) {
            ...
            if (hasFileTools) {
                toolHints.push('You have file system tools (file_list, file_read, file_write, etc.) that let you access files and folders on the server. USE THEM when the user asks about files, folders, or drives. Do NOT say you cannot access files — you CAN via these tools.');
            }
            if (hasShellTools) {
        ...
        const streamOptions: Parameters<typeof streamText>[0] = {
            ...
            ...(aiTools ? { stopWhen: stepCountIs(Object.keys(tools).includes('coding_cli') ? 15 : 10) } : {}),
        };
        ...
        const result = streamText({
            ...
            onStepFinish: (step) => {
                ...
                try {
                    ...
                    console.log(`[Chat] Step ${stepCounter} finished:`, {
                        ...
                        text: step.text ? (step.text.substring(0, 100) + (step.text.length > 100 ? '...' : '')) : '',
            ...
            onFinish: async (completion) => {
                try {
                    console.log(`[Chat] Stream finished after ${stepCounter} step(s). Text length: ${completion.text?.length || 0}`);
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    ...
                    db.insert(messagesTable).values({
                        ...
                        metadata: {
                            ...
                            ragSources: ragSources.length > 0 ? ragSources.map(s => ({
